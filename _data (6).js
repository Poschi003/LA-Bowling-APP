...
const defaultData = {
  settings: {
    ...
    adminEmployees: [],
    terminalCode: process.env.DEFAULT_TERMINAL_CODE || "2468",
...
function mergeData(value) {
  ...
  const merged = {
    ...
    dayReports: value?.dayReports || base.dayReports,
...
function ensureRequiredPositions(positions) {
  ...
  if (!clean.some((position) => normalizePosition(position) === "mechanik")) clean.push("Mechanik");
  return clean;
}

function normalizePosition(position) {
  return String(position || "").trim().toLowerCase().replace(/\s+\d+$/, "");
}
...
function verifyTerminal(settings, code) {
  return verifyPin(code, settings.terminalCodeHash || settings.terminalCode);
}
